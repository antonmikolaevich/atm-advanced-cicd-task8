pipeline {//for jenkins -docker integration build image for the beginning
    ...
    stage ("Run tests in Chrome"){
        steps {
            script {
            bat "docker run -d --net grid -e SE_EVENT_BUS_HOST=selenium-hub -e SE_EVENT_BUS_PUBLISH_PORT=4442 -e SE_EVENT_BUS_SUBSCRIBE_PORT=4443 -v %cd%\\reporterDocker:/api-project/reporterDocker selenium/node-chrome:3.141.59-20210929"
